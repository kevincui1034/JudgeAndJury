# Deploy-Time Correctness Failure Taxonomy v0.3
...
## Intent categories (v0.3 — labels, not failure classes)

Claim-of-done checkpoints (ROADMAP-intent.md) label how a user *corrects*
an agent's claimed-done work. These are correction categories on
checkpoint records — a separate axis from the failure classes above:
they never appear in `checks`, never block, and exist so repeated
same-category corrections can graduate into candidate preferences
(`proofjury prefs`). Deliberately small — categories are graduation
signatures, and a sprawling set never accumulates the repeats graduation
needs.

| Category | The correction says… |
|---|---|
| `decomposition` | split it up / structure it differently |
| `size` | smaller files, shorter functions |
| `style_fidelity` | the visual/stylistic result isn't what was wanted |
| `framework_choice` | wrong library/tool/framework/idiom |
| `naming` | naming conventions |
| `scope` | did more or less than asked |
| `other` | anything else (never graduates) |

## Versioning

This is v0.3 of an open spec (v0.3 adds the intent-category axis; the
nine failure classes are unchanged from v0.2). Additions must be additive
(new classes, new evidence fields); renames or semantic changes to
existing classes require a major version bump, since persisted memory
records reference these class names verbatim. Intent categories follow
the same rule — checkpoint records and preference files reference them
verbatim.
