...
def memory_stats(
    ...
    intent = data["intent"]
    if intent["checkpoints"]:
        rate = intent["correction_rate"]
        table.add_row(
            "intent checkpoints",
            f"{intent['checkpoints']} checkpoint(s) · outcomes: "
            + (", ".join(f"{k}={v}" for k, v in sorted(intent["outcomes"].items())) or "—")
            + (f" · correction rate {rate:.0%}" if rate is not None else ""),
        )
    if intent["prefs"]:
        table.add_row(
            "preferences",
            ", ".join(f"{k}={v}" for k, v in sorted(intent["prefs"].items()))
            + " (proofjury prefs list)",
        )
    ledger = data["ledger"]
    table.add_row(
        "judge spend",
...
def status() -> None:
    ...
    if (root / ".proofjury").is_dir():
        ...
        from .checkpoint import get_store as _ckpt_store

        ckpts = sum(1 for _ in _ckpt_store(root).iter_records())
        if ckpts:
            console.print(f"  {ckpts} checkpoint(s) in .proofjury/checkpoints.jsonl")
    else:
...
#: Claude Code events proofjury wires: the PreToolUse gate plus the three
#: intent-alignment events (ROADMAP-intent.md). Only PreToolUse takes a
#: matcher — the others fire on the event itself.
_CLAUDE_HOOK_EVENTS: list[tuple[str, str | None, str]] = [
    ("PreToolUse", "Bash", "proofjury hook"),
    ("Stop", None, "proofjury hook --event stop"),
    ("UserPromptSubmit", None, "proofjury hook --event prompt"),
    ("SessionStart", None, "proofjury hook --event session-start"),
]


def _merge_claude_hook(root: Path) -> str:
    """Write proofjury's hooks into .claude/settings.json (merge, don't
    clobber). Idempotent per event — re-running init on an already-wired
    repo adds only the events that are missing, so the intent events flow
    to existing users through `proofjury init`."""
    ...
    added: list[str] = []
    for event, matcher, command in _CLAUDE_HOOK_EVENTS:
    ...
    return f"wrote {', '.join(added)} hook(s) → {settings_path}"
...
def _render_proofjury_toml(extra_patterns: list[str]) -> str:
    ...
    literals (no escaping needed for regex backslashes); the key is
    inserted INSIDE ``[hook]`` — i.e. before the ``[checkpoint]`` section
    that follows it in the template. A pattern containing a single quote
    can't sit in a literal string (TOML has no escaping inside ``'...'``),
    so it's emitted as a basic string with JSON escaping — valid TOML for
    these inputs.
    """
    ...
    rendered_block = "\n".join(block) + "\n"
    marker = "\n[checkpoint]"
    if marker in PROOFJURY_TOML_TEMPLATE:
        return PROOFJURY_TOML_TEMPLATE.replace(
            marker, rendered_block + marker, 1
        )
    return PROOFJURY_TOML_TEMPLATE + rendered_block
...
# --------------------------------------------------------------------------
# checkpoint / intent-worker / prefs — intent alignment (ROADMAP-intent.md)
# --------------------------------------------------------------------------
...
def checkpoint(
    event: str = typer.Option(
        "manual",
        "--event",
        help="manual | commit (wire `proofjury checkpoint --event commit` "
        "into a git post-commit hook for agent-agnostic checkpoints).",
    ),
    task: str = typer.Option(
        None, "--task", help="What this chunk of work was supposed to do."
    ),
) -> None:
    """Record a claim-of-done checkpoint for the current worktree."""
    from .checkpoint import record_checkpoint

    event = (event or "manual").strip().lower()
    if event not in ("manual", "commit"):
        _usage_error("--event must be one of: manual, commit")
    env = dict(os.environ)
    record = record_checkpoint(
        Path.cwd(), env, event=event, task=task or env.get("PROOFJURY_TASK")
    )
    console = Console(highlight=False)
    if record is None:
        console.print(
            "[dim]no checkpoint — no new work since the last one (or "
            "checkpoints are disabled / rate-capped)[/dim]"
        )
        return
    console.print(
        f"checkpoint {record['id']} → .proofjury/checkpoints.jsonl "
        f"({record['diff_lines']} diff line(s), {len(record['changed_files'])} file(s))"
    )


@app.command(name="intent-worker", hidden=True)
def intent_worker(
    action: str = typer.Argument(..., help="classify | review"),
    target: str = typer.Argument(None, help="checkpoint id (review)"),
    session: str = typer.Option("", "--session", help="agent session id (classify)"),
) -> None:
    """Detached background worker spawned by the stop/prompt hooks.

    ``classify`` reads the raw prompt from STDIN (memory only — the
    raw-prompt privacy invariant). Always exits 0: a worker failure is an
    unlabeled checkpoint, never a user-visible error.
    """
    from .checkpoint import classify_and_label, run_intent_review

    try:
        env = dict(os.environ)
        if action == "classify":
            prompt = sys.stdin.read()
            classify_and_label(Path.cwd(), env, prompt, session or None)
        elif action == "review" and target:
            run_intent_review(Path.cwd(), env, target)
    except Exception:
        pass
    raise typer.Exit(0)
...
prefs_app = typer.Typer(
    help="Learned preferences: list, approve, reject, add, rm. Only "
    "ACTIVE (approved) preferences are ever injected into agent context."
)
app.add_typer(prefs_app, name="prefs")


def _pref_stores() -> list:
    from .memory import prefs as prefs_module

    return [prefs_module.repo_store(Path.cwd()), prefs_module.user_store()]


def _find_pref(pref_id: str):
    for store in _pref_stores():
        pref = store.get(pref_id)
        if pref is not None:
            return store, pref
    return None, None


@prefs_app.command(name="list")
def prefs_list() -> None:
    """All preferences, repo scope first."""
    console = Console(highlight=False)
    table = Table(box=box.SIMPLE)
    for column in ("id", "scope", "status", "category", "statement"):
    ...
    if count:
        console.print(table)
        console.print(
            "[dim]approve with `proofjury prefs approve <id>` — only active "
            "preferences reach your agent[/dim]"
        )
    else:
        console.print(
            "[dim]no preferences yet — they graduate from repeated "
            "corrections, or add one: proofjury prefs add \"...\"[/dim]"
        )


@prefs_app.command(name="approve")
def prefs_approve(pref_id: str = typer.Argument(...)) -> None:
    """Activate a preference — it will be injected at session start."""
    store, pref = _find_pref(pref_id)
    if pref is None:
        _usage_error(f"no preference {pref_id}")
    store.set_status(pref_id, "active", now=now_iso())
    Console(highlight=False).print(f"{pref_id} active — injected from the next session on")


@prefs_app.command(name="reject")
def prefs_reject(pref_id: str = typer.Argument(...)) -> None:
    """Reject a preference; its category never re-graduates in that scope."""
    store, pref = _find_pref(pref_id)
    if pref is None:
        _usage_error(f"no preference {pref_id}")
    store.set_status(pref_id, "rejected", now=now_iso())
    Console(highlight=False).print(f"{pref_id} rejected — this category will not re-graduate")


@prefs_app.command(name="add")
def prefs_add(
    statement: str = typer.Argument(..., help="The preference, stated plainly."),
    category: str = typer.Option("other", "--category"),
    user_scope: bool = typer.Option(
        False, "--user", help="User scope (all your repos) instead of this repo."
    ),
) -> None:
    """State a preference directly — no need to wait for graduation.
    Added as ACTIVE: saying it explicitly IS the approval."""
    from .memory import prefs as prefs_module

    store = prefs_module.user_store() if user_scope else prefs_module.repo_store(Path.cwd())
    pref = store.add(statement, category, status="active", now=now_iso())
    Console(highlight=False).print(f"{pref['id']} active ({store.scope} scope)")


@prefs_app.command(name="rm")
def prefs_rm(pref_id: str = typer.Argument(...)) -> None:
    """Delete a preference entirely (use reject to keep the suppression)."""
    store, pref = _find_pref(pref_id)
    if pref is None:
        _usage_error(f"no preference {pref_id}")
    store.remove(pref_id)
    Console(highlight=False).print(f"{pref_id} removed")
...
def hook(
    ...
    event: str = typer.Option(
        "pretooluse",
        "--event",
        help="pretooluse | stop | prompt | session-start (intent events are "
    ...
    settings.json files (default agent: claude, event: pretooluse).
    """
    ...
    event = (event or "pretooluse").strip().lower()
    if event not in ("pretooluse", "stop", "prompt", "session-start"):
        _usage_error("--event must be one of: pretooluse, stop, prompt, session-start")

    if event != "pretooluse":
        # Intent events (ROADMAP-intent.md) fail OPEN — observation and
        # context delivery must never disturb the agent's turn. The
        # opposite polarity of the PreToolUse gate below, which fails
        # closed because it stands between a broken deploy and prod.
        raw = sys.stdin.read()
        try:
            payload = json.loads(raw) if raw.strip() else {}
            handler = {
                "stop": handle_stop_hook,
                "prompt": handle_prompt_hook,
                "session-start": handle_session_start_hook,
            }[event]
            output = handler(payload, Path.cwd(), dict(os.environ))
        except Exception as exc:
            sys.stderr.write(f"proofjury hook ({event}) internal error — ignored: {exc}\n")
            output = {}
