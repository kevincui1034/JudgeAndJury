...
def _active_preference_statements(root: Path, env) -> list[str]:
    """ACTIVE learned-preference statements for the advisory prompt
    (ROADMAP-intent.md I4). Best-effort: any error → no section, and the
    prompt is byte-identical to a pre-preference run."""
    try:
        from .config import prefs_enabled
        from .memory import prefs as prefs_module

        if not prefs_enabled(load_config(root), env):
            return []
        return [
            p["statement"]
            for p in prefs_module.active_prefs(root, env)
            if p.get("statement")
        ]
    except Exception:
        return []
