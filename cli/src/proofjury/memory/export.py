...
def stats(store: MemoryStore, ledger_path: Path, *, env=None) -> dict:
    ...
    return {
        ...
        # Intent alignment (ROADMAP-intent.md): checkpoint volume, outcome
        # labels, and the product's own success metric — correction rate
        # over labeled checkpoints (I2 passive phase = the baseline).
        "intent": intent_stats(store.root, env),
    }


def intent_stats(proofjury_dir: Path, env=None) -> dict:
    """Checkpoint + preference counts. Zeroes when the pillar is unused —
    stats must never fail because intent files are absent."""
    checkpoints = 0
    ...
    try:
        ...
        for record in CheckpointStore(proofjury_dir).iter_records():
            checkpoints += 1
            findings += len(record.get("findings") or [])
            outcome = record.get("outcome") or {}
            label = outcome.get("label") or "unknown"
            outcomes[label] = outcomes.get(label, 0) + 1
    except Exception:
        pass
    corrected = outcomes.get("corrected", 0)
    labeled = corrected + outcomes.get("accepted_implicit", 0)
    prefs_by_status: dict[str, int] = {}
    try:
        from . import prefs as prefs_module

        root = Path(proofjury_dir).parent
        for pref in prefs_module.iter_all(root, env):
            status = pref.get("status", "candidate")
            prefs_by_status[status] = prefs_by_status.get(status, 0) + 1
    except Exception:
        pass
    return {
        ...
        "outcomes": outcomes,
        "findings": findings,
        "correction_rate": (corrected / labeled) if labeled else None,
        "prefs": prefs_by_status,
    }
